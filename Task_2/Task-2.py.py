...
root['bg'] = '#E6E6E6'  # встановлює колір
root.title('Калькулятор')
root.wm_attributes('-alpha', 0.95)
root.geometry('482x568+450+90')  # встановлює розміри вікна програми
root.resizable(height=False,width=False)  # робить так, що користувач не може змінювати розміри вікна програми
calc = Frame(root)
...
class Calc():  # клас полегшує роботу, тобто використання об'єкта дозволяє легко розділити код на модульні модулі
    def __init__(self):  # належить до групи методів перевантаження операторів
        self.total = 0
        self.current = ''
    ...
    def display(self,
                value):  # те, що буде показуватися після запуска калькулятора й після деяких операцій(коли буде використовуватися .display)
    ...
    def backspace(self):  # видалення по одному елементу в дисплеї
        if self.current == float(
                self.current):  # помилка, бо описує об'єкти, які є "containers", тобто вони містять інші об'єкти (subscriptable)
            tkinter.messagebox.showerror("Калькулятор", "Помилка!")
        else:
    ...
    def Clear_Entry(
            self):  # очищення дисплея від введеного значення(попередні введені значення при цьому зберігаються через False в self.result та True в input_value
        self.result = False
        self.current = "0"
        self.display(0)
        self.input_value = True
    ...
    def Clear_All_Entry(
            self):  # очищення дисплея від усіх введених значень, бо self.total=0(але в дисплеї залишається число 0 для заміни результату попередніх операцій)
        self.Clear_Entry()
        self.total = 0
    ...
    def cos(
            self):  # визначається функція для косинуса(обчислення проводяться за допомогою функцій сos та radians(який перетворює кут txtDisplay, значення, яке ми отримуємо з дисплея, з градусів у радіани) з модуля math)
        self.result = False
        self.current = math.cos(math.radians(float(txtDisplay.get())))
        self.display(self.current)

    def sin(
            self):  # визначається функція для синуса(обчислення проводяться за допомогою функції sin та radians з модуля math)
        self.result = False
        self.current = math.sin(math.radians(float(txtDisplay.get())))
        self.display(self.current)

    def tan(
            self):  # визначається функція для тангенса(обчислення проводяться за допомогою функції tan та radians з модуля math)
        self.result = False
        self.current = math.tan(math.radians(float(txtDisplay.get())))
        self.display(self.current)

    def ctg(
            self):  # визначається функція для котангенса(обчислення проводяться за допомогою функцій cos та sin(щоб отримати котангенс, то(з тригонометричних формул) потрібно cos(x) поділити на sin(x)) з модуля math)
        self.result = False
        self.current = math.cos(math.radians(float(txtDisplay.get()))) / math.sin(math.radians(float(txtDisplay.get())))
        self.display(self.current)

    def ln(
            self):  # визначається функція для натурального логарифма(обчислення проводяться за допомогою функції log з модуля math)
        self.result = False
        self.current = math.log(float(txtDisplay.get()))
        self.display(self.current)

    def log10(
            self):  # визначається функція для десяткового логарифма(обчислення проводяться за допомогою функції log10 з модуля math)
        self.result = False
...
for j in range(2, 5):
    for k in range(3):
        btn.append(Button(calc, width=6, height=2, bg='#FAFAFA', activebackground='#B2AEB5', fg='black',
                          font=('Helvetica', 20, 'bold'), bd=1, relief="raised", text=numbers[i]))
        btn[i].grid(row=j, column=k, pady=1)
        btn[i]["command"] = lambda x=numbers[i]: added_value.enter_number(x)
        i += 1
# Оформлення кнопок для основних функцій у калькуляторі :

btnClearEntry = Button(calc, text=chr(67) + chr(69), width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5',
                       fg='black', font=('Helvetica', 20, 'bold'), bd=1, relief="groove",
                       command=added_value.Clear_Entry)
btnClearEntry.grid(row=1, column=0,
                   pady=12)  # віджет розділений на число рядків і стовпців, а також присутній зовнішній відступ між віджетами
btnClearEntry.bind("<Enter>",
                   button_hover)  # функція прив'язки використовується для обробки подій, подія "<Enter>" наступає тоді, коли вказівник миші увійшов у віджет
btnClearEntry.bind("<Leave>", button_hover_leave)  # подія "<Leave>" наступає тоді, коли вказівник миші залишає віджет

# Для кожної кнопки :
btnClearAll = Button(calc, text=chr(67), width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                     font=('Helvetica', 20, 'bold'), bd=1, relief="groove", command=added_value.Clear_All_Entry)
btnClearAll.grid(row=1, column=1, pady=1)
btnClearAll.bind("<Enter>", button_hover1)
btnClearAll.bind("<Leave>", button_hover_leave1)

btnClearLast = Button(calc, text="\u2190", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                      font=('Helvetica', 20, 'bold'), bd=1, relief="groove", command=added_value.backspace)
btnClearLast.grid(row=1, column=2, pady=1)
btnClearLast.bind("<Enter>", button_hover2)
btnClearLast.bind("<Leave>", button_hover_leave2)

btnbin = Button(calc, text="bin", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20), bd=1, relief="groove", command=added_value.convert_to_bin)
btnbin.grid(row=1, column=3, pady=1)
btnbin.bind("<Enter>", button_hover3)
btnbin.bind("<Leave>", button_hover_leave3)

btnAdd = Button(calc, text="+", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20), bd=1, relief="groove", command=lambda: added_value.operation("add"))
btnAdd.grid(row=2, column=3, pady=1)
btnAdd.bind("<Enter>", button_hover4)
btnAdd.bind("<Leave>", button_hover_leave4)

btnSub = Button(calc, text="-", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20), bd=1, relief="groove", command=lambda: added_value.operation("sub"))
btnSub.grid(row=3, column=3, pady=1)
btnSub.bind("<Enter>", button_hover5)
btnSub.bind("<Leave>", button_hover_leave5)

btnMul = Button(calc, text="x", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20), bd=1, relief="groove", command=lambda: added_value.operation("multi"))
btnMul.grid(row=4, column=3, pady=1)
btnMul.bind("<Enter>", button_hover6)
btnMul.bind("<Leave>", button_hover_leave6)

btnDiv = Button(calc, text="/", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20), bd=1, relief="groove", command=lambda: added_value.operation("divide"))
btnDiv.grid(row=5, column=3, pady=1)
btnDiv.bind("<Enter>", button_hover7)
btnDiv.bind("<Leave>", button_hover_leave7)

btnDot = Button(calc, text=".", width=6, height=2, bg='#FAFAFA', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20, 'bold'), bd=1, relief="raised", command=lambda: added_value.enter_number("."))
btnDot.grid(row=5, column=0, pady=1)
btnDot.bind("<Enter>", button_hover8)
btnDot.bind("<Leave>", button_hover_leave8)

btnZero = Button(calc, text="0", width=6, height=2, bg='#FAFAFA', activebackground='#B2AEB5', fg='black',
                 font=('Helvetica', 20, 'bold'), bd=1, relief="raised", command=lambda: added_value.enter_number(0))
btnZero.grid(row=5, column=1, pady=1)

btnEquals = Button(calc, text="=", width=6, height=2, bg='#DBC99A', activebackground='#CC9E29', fg='black',
                   font=('Helvetica', 20, 'bold'), bd=1, relief="raised", command=added_value.overall_value)
...
# Рядок 0:
btnsin = Button(calc, text="sin", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20, 'bold'), bd=1, relief="groove", command=added_value.sin)
btnsin.grid(row=0, column=4, pady=1)
btnsin.bind("<Enter>", button_hover10)
btnsin.bind("<Leave>", button_hover_leave10)

btncos = Button(calc, text="cos", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20, 'bold'), bd=1, relief="groove", command=added_value.cos)
btncos.grid(row=0, column=5, pady=1)
btncos.bind("<Enter>", button_hover11)
btncos.bind("<Leave>", button_hover_leave11)

# Рядок 1:
btntan = Button(calc, text="tan", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20, 'bold'), bd=1, relief="groove", command=added_value.tan)
btntan.grid(row=1, column=4, pady=1)
btntan.bind("<Enter>", button_hover12)
btntan.bind("<Leave>", button_hover_leave12)

btnctg = Button(calc, text="ctg", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20, 'bold'), bd=1, relief="groove", command=added_value.ctg)
btnctg.grid(row=1, column=5, pady=1)
btnctg.bind("<Enter>", button_hover13)
btnctg.bind("<Leave>", button_hover_leave13)

# Рядок 2:
btnlog = Button(calc, text="log", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                font=('Helvetica', 20, 'bold'), bd=1, relief="groove", command=added_value.log10)
btnlog.grid(row=2, column=4, pady=1)
btnlog.bind("<Enter>", button_hover14)
btnlog.bind("<Leave>", button_hover_leave14)

btnln = Button(calc, text="ln", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
               font=('Helvetica', 20, 'bold'), bd=1, relief="groove", command=added_value.ln)
btnln.grid(row=2, column=5, pady=1)
btnln.bind("<Enter>", button_hover15)
btnln.bind("<Leave>", button_hover_leave15)

# Рядок 3:
btnmodulo = Button(calc, text="%", width=6, height=2, bg='#F0F0F0', activebackground='#B2AEB5', fg='black',
                   font=('Helvetica', 20, 'bold'), bd=1, relief="groove", command=lambda: added_value.operation("mod"))
...
# Функції для меню
def Exit():
    Exit = tkinter.messagebox.askyesno("Калькулятор",
                                       "Чи дійсно Ви хочете вийти?")  # askyesno використовується для підказки користувачам із параметрами «так» або «ні», тобто boolean значення
    if Exit > 0:
        root.destroy()
        return
